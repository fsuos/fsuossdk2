...
class {{ Project.Name }}: public UniDataDevice<{{ Project.Name }}_Data_t, SMDAIDevice, RT_{{ Project.Name }}>
{
public:
    {{ Project.Name }}();
    ~{{ Project.Name }}();
    bool InitSetting(const Json::Value&  settingRoot);
    float process(short ai);
...
PLUMA_INHERIT_PROVIDER({{ Project.Name }}, SMDAIDevice);
#endif // WATERSENSOR_H

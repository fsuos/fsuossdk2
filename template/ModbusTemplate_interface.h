...
struct {{ Project.Name }}_Data_t {
    ...
    {% if tsc.CmdGroupStart is defined %}
        ...
        {% for sc in tsc.CmdGroupSample %}
            ...
            uint16_t {{  "r%d_%d_%d[%d][%d];"%(sc.Cmd, tsc.CmdGroupStart, sc.Offset,(tsc.CmdGroupEnd-tsc.CmdGroupStart)/tsc.CmdGroupStep, sc.Len) }}
            {% elif sc.Cmd == 1 or sc.Cmd == 2 %}
            uint8_t {{  "b%d_%d_%d[%d][%d];"%(sc.Cmd, tsc.CmdGroupStart, sc.Offset, (tsc.CmdGroupEnd-tsc.CmdGroupStart)/tsc.CmdGroupStep, sc.Len) }}
            {% endif %}
        {% endfor %}

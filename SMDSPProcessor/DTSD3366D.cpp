...
bool DTSD3366D::RefreshStatus()
{
    ...
    state = DTSD3366D_R3_9;
    modbus_read_registers(9, 2);
    return true;
}
...
bool DTSD3366D::process_payload(enum tab_type type, size_t len)
{
    switch(state){
      case DTSD3366D_R3_9:{
            memcpy(cData.r3_9, tab_reg, sizeof(uint16_t)*2);
            state = DTSD3366D_R3_366;
            modbus_read_registers(366, 12);
	    break;
      }
      case DTSD3366D_R3_366:{
            memcpy(cData.r3_366, tab_reg, sizeof(uint16_t)*12);
            state = DTSD3366D_R3_378;
            modbus_read_registers(378, 24);
	    break;
      }
      case DTSD3366D_R3_378:{
            memcpy(cData.r3_378, tab_reg, sizeof(uint16_t)*24);
            state = DTSD3366D_R3_402;
            modbus_read_registers(402, 8);
	    break;
      }
      case DTSD3366D_R3_402:{
            memcpy(cData.r3_402, tab_reg, sizeof(uint16_t)*8);
            state = DTSD3366D_R3_4096;
            modbus_read_registers(4096, 2);
	    break;
      }
      case DTSD3366D_R3_4096:{
            memcpy(cData.r3_4096, tab_reg, sizeof(uint16_t)*2);
            state = DTSD3366D_R3_4144;
            modbus_read_registers(4144, 2);
	    break;
      }
      case DTSD3366D_R3_4144:{
            memcpy(cData.r3_4144, tab_reg, sizeof(uint16_t)*2);
            state = DTSD3366D_R3_4192;
            modbus_read_registers(4192, 2);
	    break;
      }
      case DTSD3366D_R3_4192:{
            memcpy(cData.r3_4192, tab_reg, sizeof(uint16_t)*2);
            RoundDone();
	    return false;
      }
   }
